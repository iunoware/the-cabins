...
export default function AdminDashboard() {
  ...
  const [activeAction, setActiveAction] = useState<"product" | "category" | "project" | "review" | null>(null);
  ...
  const handleTriggerAction = (type: "product" | "category" | "project" | "review") => {
    ...
    <div className="flex flex-col gap-6 max-w-7xl mx-auto w-full">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="animate-pulse bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800/80 rounded-2xl p-6 h-36">
        ...
        <KpiSection
          data={[
            { title: "Total Products", value: 0, change: "No products added", isPositive: false, icon: null },
            { title: "Total Categories", value: 0, change: "No categories added", isPositive: false, icon: null },
            { title: "Total Projects", value: 0, change: "No projects added", isPositive: false, icon: null },
            { title: "Customer Reviews", value: 0, change: "No reviews published", isPositive: false, icon: null },
            { title: "Website Visitors", value: 0, change: "No traffic logs", isPositive: false, icon: null },
            { title: "WhatsApp Enquiries", value: 0, change: "No enquiries received", isPositive: false, icon: null },
          ]}
        />
      ) : (
        <KpiSection />
      ...
      {/* 3. Analytics Chart */}
      {isLoading ? (
        <div className="animate-pulse bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800/80 rounded-2xl p-6 h-[340px] mb-8">
          ...
          <div className="w-full h-[200px] border-b border-l border-gray-150 dark:border-zinc-800 flex items-end p-2">
            <div className="w-full h-1/2 bg-gray-100 dark:bg-zinc-850/60 rounded-t-lg animate-pulse" />
          </div>
        </div>
      ) : isEmpty ? (
        <div className="bg-white dark:bg-zinc-900 border border-gray-150 dark:border-zinc-800/80 rounded-2xl p-8 shadow-xs mb-8 text-center flex flex-col items-center justify-center min-h-[300px]">
          <HelpCircle className="text-gray-300 dark:text-zinc-700 mb-3" size={32} />
          <h4 className="text-sm font-bold text-gray-800 dark:text-gray-200">No Visitor Data Logged</h4>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 max-w-sm">
            Traffic logs will populate automatically once real visitors visit the customer website interface.
          </p>
        </div>
      ) : (
        <AnalyticsSection />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-12">
        ...
        {isLoading ? (
          <div className="animate-pulse bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800/80 rounded-2xl p-6 h-[380px]">
          ...
          <div className="bg-white dark:bg-zinc-900 border border-gray-150 dark:border-zinc-800/80 rounded-2xl p-8 shadow-xs text-center flex flex-col items-center justify-center min-h-[350px]">
            <HelpCircle className="text-gray-300 dark:text-zinc-700 mb-3" size={32} />
            <h4 className="text-sm font-bold text-gray-800 dark:text-gray-200">No Top Performing Products</h4>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 max-w-sm">
              We compile views and clicks analysis automatically based on active inquiries.
            </p>
          </div>
        ) : (
          <TopProductsSection />
        ...
        {isLoading ? (
            ...
            <div className="flex flex-col gap-6.5 pl-4 border-l border-gray-100 dark:border-zinc-800">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="flex flex-col gap-1.5 relative">
                  <div className="absolute -left-[24px] w-4.5 h-4.5 rounded-full bg-gray-200 dark:bg-zinc-850 border-4 border-white dark:border-zinc-900" />
          ...
          <div className="bg-white dark:bg-zinc-900 border border-gray-150 dark:border-zinc-800/80 rounded-2xl p-8 shadow-xs text-center flex flex-col items-center justify-center min-h-[350px]">
            <HelpCircle className="text-gray-300 dark:text-zinc-700 mb-3" size={32} />
            <h4 className="text-sm font-bold text-gray-800 dark:text-gray-200">No Recent Activities Logged</h4>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 max-w-sm">
              Any actions you make (adding/editing products and reviews) will be listed here.
            </p>
          </div>
        ) : (
      ...
      <div className="mt-8 border-t border-gray-200 dark:border-zinc-850 pt-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 font-medium">
          ...
          <span><strong>Developer Tooling:</strong> Use these toggles to review the pre-configured layout modes.</span>
        </div>
