...
export default function ProjectsSection({ projects }: ProjectsSectionProps) {
  const categories = useMemo(() => {
    return ["All Projects", ...new Set(projects.map((project) => project.category.name))];
  }, [projects]);

  const locations = useMemo(() => {
    return [
      "All Locations",
      ...new Set(projects.map((project) => project.city).filter(Boolean) as string[]),
    ];
  }, [projects]);
  ...
  const filteredProjects = useMemo(() => {
    return projects.filter((project) => {
      const matchCategory =
        activeCategory === "All Projects" || project.category.name === activeCategory;

      const matchLocation =
        activeLocation === "All Locations" || project.city === activeLocation;
