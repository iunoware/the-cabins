...
export default function TopBar() {
  ...
  return (
    ...
    >
      <div className="max-w-7xl w-full mx-auto px-4 md:px-6 flex flex-col md:flex-row items-center justify-between py-1.5 md:py-0 h-full gap-2 md:gap-0">
        {/* Left Side: Contact Info */}
        <div className="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-4 sm:gap-6 md:gap-8 w-full md:w-auto">
          {contactItems.map((item, index) => {
            const Icon = item.icon;
            const content = (
              <>
                <Icon className="w-4 h-4 text-[#EF4444] shrink-0" />
                <span className="font-sans text-[14px] font-normal text-white/80">
                  {item.text}
                </span>
              </>
            );
            ...
            if (item.href) {
              return (
                <a
                  ...
                  className="flex items-center gap-2 transition-opacity duration-200 hover:opacity-80 focus:outline-none focus:ring-1 focus:ring-[#EF4444] px-1 py-0.5 rounded"
                >
                  {content}
                </a>
              );
